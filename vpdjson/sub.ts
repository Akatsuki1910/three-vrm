...
function returnBonePositionH(vrm: any, boneSt: string): THREE.Vector3 {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const movingTarget1 = new THREE.Mesh(
    new THREE.SphereGeometry(0.1),
    new THREE.MeshBasicMaterial({
      color: 0xff0000,
    })
  )
  const movingTarget2 = new THREE.Mesh(
    new THREE.SphereGeometry(0.1),
    new THREE.MeshBasicMaterial({
      color: 0x00ff00,
    })
  )
  const movingTarget3 = new THREE.Mesh(
    new THREE.SphereGeometry(0.1),
    new THREE.MeshBasicMaterial({
      color: 0x0000ff,
    })
    ...
    (gltf) => {
      VRM.from(gltf).then((vrm) => {
        ...
        movingTarget.position.set(
          returnBonePositionH(vrm, 'LeftFoot').x,
          returnBonePositionH(vrm, 'LeftFoot').y,
          returnBonePositionH(vrm, 'LeftFoot').z
        )
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (i == 0) {
            bone.position.set(
              returnBonePositionH(vrm, boneName).x,
              returnBonePositionH(vrm, boneName).y,
              returnBonePositionH(vrm, boneName).z
            )
          } else {
            bone.position.set(
              returnBoneH(vrm, boneName).position.x,
              returnBoneH(vrm, boneName).position.y,
              returnBoneH(vrm, boneName).position.z
            )
          }
          if (bones[i - 1]) {
            bones[i - 1].add(bone)
          ...
          let constraints =
            i == 0 ? [new IKBallConstraint(270)] : [new IKBallConstraint(90)]
        ...
        returnBoneH(vrm, 'LeftUpperLeg').rotation.set(
          bones[0].rotation.z - pz,
          bones[0].rotation.y - py,
          bones[0].rotation.x - px
        )
  ...
  const update = () => {
    ...
    // legBone.Y == IKBone.Z
    // legBone.Z == IKBone.Y
    returnBoneH(vrmupdate, 'LeftUpperLeg').rotation.x =
